...
function robustnessAnalysis_Responses_plotAndStore(data, Ps, defaultData, r, responseName, outputPrefix, xLabel, yLabel, yLim)
...
% 'r' in this case is the index into the desired response data in the 'data' variable. IE, starts from 2, since the first 'r' in data corresponds
% to the run number. 
% 'responseName' is the name of the response for which this graph is being generated. It is used in the filename. 
% 'yLim' can be set to a non-zero value to set the y-axis upper limit. It may  also be left at zero by 
% default
%
clf;
...
xlabel(xLabel,'FontSize',fontsize);                                           % add the axis labels. 
...
pos = pos + [0 -15 0];                                                   % whenever a different font size is desired, this is going to have to change also. 
        % copy across relevant attributes from the xlabel text box. 
text(pos(1), pos(2), xLabel,'FontSize',fontsize,'Units',get(xlabh,'Units'),'VerticalAlignment',get(xlabh,'VerticalAlignment'),'HorizontalAlignment',get(xlabh,'HorizontalAlignment'));
xlabel({''});                                                           % remove the xlabel box text. 
%--------------------------------

ylabel(yLabel,'FontSize',fontsize);                                           % much simpler, if only it worked for the axis too!
