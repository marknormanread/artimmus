...
function robustnessAnalysis_Responses_plotAndStore(data, Ps, defaultData, r, responseName, outputPrefix, xLabel, yLabel)
...
Ys = zeros( size(data(1).data,2), length(data) );                % first dimension is the length of data samples, the second is the number of parameters

for p = 1:length(data)                                              % scan all parameter values. 
  if strcmp(r,'max')
    Ys(:,p) = data(p).data(2,:);                                    % first column is time of sample
    if data(p).AtestMaxSignificant
      colours(p) = 'r';
    else
      colours(p) = 'b';
    end
    if data(p).paramVal == defaultData.paramVal
      colours(p) = 'k';
    end

  else

    Ys(:,p) = data(p).data(3,:);                                    % first column is time of sample
    if data(p).Atest40Significant
      colours(p) = 'r';
    else
      colours(p) = 'b';
    end
